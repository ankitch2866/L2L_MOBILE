...
import { createPLC, fetchPLCs } from '../../../store/slices/plcSlice';

const AddPLCScreen = ({ navigation }) => {
  const dispatch = useDispatch();
  const { theme } = useTheme();
  const [loading, setLoading] = useState(false);
  const [valueType, setValueType] = useState('percentage'); // 'percentage' or 'amount'
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert('Success', 'PLC created successfully', [
        { text: 'OK', onPress: () => {
          dispatch(fetchPLCs());
          navigation.goBack();
        }}
      ...
      Alert.alert('Error', error || 'Failed to create PLC');
    } finally {
      setLoading(false);
    }
  };
    ...
    <ScrollView style={[styles.container, { backgroundColor: '#F9FAFB' }]}>
      <View style={styles.form}>
        ...
        <Card style={[styles.section, styles.greenSection]}>
          <Card.Content>
            ...
            <Text variant="bodySmall" style={styles.sectionSubtitle}>
              Enter the PLC name and details
            </Text>
        ...
        <Button
          ...
          icon="content-save"
        >
          {loading ? 'Creating PLC...' : 'Create PLC'}
        </Button>
      </View>
    </ScrollView>
  );
