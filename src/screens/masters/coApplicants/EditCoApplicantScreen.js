...
import { fetchCoApplicantById, updateCoApplicant } from '../../../store/slices/coApplicantsSlice';
import { fetchProjects } from '../../../store/slices/projectsSlice';
import { fetchCustomers } from '../../../store/slices/customersSlice';

const EditCoApplicantScreen = ({ route, navigation }) => {
  const { id } = route.params;
  const dispatch = useDispatch();
  const { theme } = useTheme();
  const { current, loading: coApplicantLoading } = useSelector(state => state.coApplicants);
  ...
  useEffect(() => {
    if (current) {
      setFormData({
        customer_id: current.customer_id?.toString() || '',
        project_id: current.project_id?.toString() || '',
        is_co_applicant: current.is_co_applicant || false,
        name: current.name || '',
        guardian_relation: current.guardian_relation || '',
        ...
        income_tax_ward_no: current.income_tax_ward_no || '',
        dist_no: current.dist_no || '',
        pan_no: current.pan_no || '',
        dob: current.dob ? current.dob.split('T')[0] : '',
        date_of_agreement: current.date_of_agreement ? current.date_of_agreement.split('T')[0] : '',
        nationality: current.nationality || 'Resident India',
        custom_nationality: ''
      });
    }
  }, [current]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await dispatch(updateCoApplicant({ id, data: submissionData })).unwrap();
      Alert.alert('Success', 'Co-Applicant updated successfully');
      navigation.goBack();
    } catch (error) {
      Alert.alert('Error', error || 'Failed to update co-applicant');
    } finally {
      setLoading(false);
    }
  };

  if (coApplicantLoading || !formData) return <LoadingIndicator />;
...
export default EditCoApplicantScreen;
