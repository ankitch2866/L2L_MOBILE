...
import { fetchBrokerById, updateBroker } from '../../../store/slices/brokersSlice';
import { fetchProjects } from '../../../store/slices/projectsSlice';

const EditBrokerScreen = ({ route, navigation }) => {
  const { id } = route.params;
  const dispatch = useDispatch();
  const { theme } = useTheme();
  const { current, loading: brokerLoading } = useSelector(state => state.brokers);
  ...
  useEffect(() => {
    if (current) {
      setFormData({
        ...
        net_commission_rate: current.net_commission_rate?.toString() || '',
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await dispatch(updateBroker({ id, data: submissionData })).unwrap();
      Alert.alert('Success', 'Broker updated successfully');
      navigation.goBack();
    } catch (error) {
      Alert.alert('Error', error || 'Failed to update broker');
    } finally {
      setLoading(false);
    }
  };

  if (brokerLoading || !formData) return <LoadingIndicator />;
